...
GLvoid CBitmapFont::DrawTextFormat( GLfloat fX, GLfloat fY, LPCTSTR fmt, ... )
{
	...
	glListBase( m_uiBase );
	glCallLists( static_cast< GLsizei >( CUHF::GetStringLength( cText ) ), GL_UNSIGNED_SHORT, cText );
	glPopAttrib();
...
GLvoid CBitmapFont::DrawText( GLfloat fX, GLfloat fY, LPCTSTR fmt )
{
	...
	glPushAttrib( GL_LIST_BIT );
	glListBase( m_uiBase );
	glCallLists( static_cast<GLsizei>( CUHF::GetStringLength( fmt ) ), GL_UNSIGNED_SHORT, fmt );
	glPopAttrib();
