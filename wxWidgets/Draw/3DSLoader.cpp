...
GLboolean C3DSLoader::ReLoad3DSObject( LPCTSTR lpFileName )
{
	...
	if( _tcslen( lpFileName ) > 0 && Load3DSObject( lpFileName ) )
	{
		CreateVertexArrays();
		return GL_TRUE;
	}
...
{
	FILE *hFile = NULL;
	errno_t error = _tfopen_s( &hFile, lpFileName, _T("rb") );
	if( error ) {
...
//		ComputeVertexNormals(); // obliczamy wektory normalne dla wierzcholkow
	...
	for( GLuint i = 0; i < _tcslen( lpFileName ); ++i )
...
TCHAR* C3DSLoader::GetActualFileNameWithoutExt()
{
	...
	GLint iSize = static_cast< GLint >( _tcslen( m_cFileNameWithoutExt ) );
