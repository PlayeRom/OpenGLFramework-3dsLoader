...
GLvoid CProfiler::Begin( LPCTSTR lpComment )
{
	SBegin sBegin;
	if( lpComment ) {
		size_t uiSize = CUHF::GetStringLength( lpComment ) + 1;
		if( uiSize >= MAX_SIZE_OF_STRING )
			uiSize = MAX_SIZE_OF_STRING - 1;
		CUHF::CopyStringToString( sBegin.lpComment, uiSize, lpComment );
	}
	else
...
GLvoid CProfiler::End()
{
	...
	if( m_aBeginMs[ iIndex ].lpComment[ 0 ] ) {
		size_t uiSize = CUHF::GetStringLength( m_aBeginMs[ iIndex ].lpComment ) + 1;
		if( uiSize >= MAX_SIZE_OF_STRING )
			uiSize = MAX_SIZE_OF_STRING - 1;
		CUHF::CopyStringToString( sSaveToLog.lpComment, uiSize, m_aBeginMs[ iIndex ].lpComment );
	}
	else
