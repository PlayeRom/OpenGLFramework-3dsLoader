...
{
	GLint iLength = static_cast< GLint >( CUHF::GetStringLength( lpFileName ) );
	if( iLength > 3 ) {
...
GLboolean CTextureLoader::LoadBMP( STexture &out_sTexture, LPCTSTR lpFileName )
{
	...
	FILE *hFile = NULL;
	if( CUHF::OpenFile( &hFile, lpFileName, _T("rb") ) != 0 )
		return GL_FALSE;
								 ...
								 GLboolean bEmbossBump /*= GL_FALSE*/ )
{
	...
	size_t uiSize = CUHF::GetStringLength( lpFileName ) + 1;
	m_aTexture[ iIndex ].lpFileName = new TCHAR[ uiSize ];
	CUHF::CopyStringToString( m_aTexture[ iIndex ].lpFileName, uiSize, lpFileName );
...
GLint CTextureLoader::CreateEmbossBumpTexture( GLint iIndex, STexture &texTexture, GLint iMode )
{
	...
	CUHF::CopyStringToString( m_aTexture[ iIndexInvert ].lpFileName, uiSize, lpFileName );
										  ...
										  GLint iFormat1 /* = 0 */, GLint iFormat2 /* = 0 */ )
{
	...
	m_aTexture[ iIndex ].lpFileName = new TCHAR[ CUHF::GetStringLength( lpFileName1 ) + CUHF::GetStringLength( lpFileName2 ) + 2 ];
										  ...
										  GLint iMode /* = 0 */, GLint iFormat /* = 0 */ )
{
	...
	m_aTexture[ iIndex ].lpFileName = new TCHAR[ CUHF::GetStringLength( lpFileNameMask ) + 20 ];
